...
class Controller_Qiniu extends Controller_Website {
    ...
    public function action_del() {
        $id = Arr::get($_GET, 'id');
        $uploadModel = Model::factory('upload_qiniu');
        $row = $uploadModel->getRowById($id);
        $url_info = parse_url($row['file_src']);
        $key = ltrim($url_info['path'], '/');
        
        $deleted = $this->_del($key);
        if ($deleted) {
    ...
    public function action_del_hc51() {
        $id = Arr::get($_GET, 'id');
        $uploadModel = Model::factory('upload_qiniu');
        $row = $uploadModel->getRowById($id);
        $url_info = parse_url($row['file_src']);
